...
class _CreateBillState extends State<CreateBill> {
  ...
  double get _totalBill =>
      _billItems.fold(0.0, (sum, item) => sum + item.price);
                  ...
                  onPressed: () async {
                    if (selectedContactToBind != null) {
                      final updatedNames = List<String>.from(
                        selectedContactToBind!.otherNames,
                      )..add(newName);
                      final updatedContact = selectedContactToBind!.copyWith(
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(
                            'ผูก "$newName" กับ ${updatedContact.mainName} แล้ว',
                          ),
                        ),
                      );
                      Navigator.pop(context);
  ...
  void _showSummary() async {
    if (_billNameController.text.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('โปรดตั้งชื่อบิล')));
      return;
    }
    ...
    if (_billItems.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('โปรดเพิ่มรายการอาหารก่อน')));
      return;
    }
  ...
  BillCalculation _calculateSummary() {
    ...
    for (var item in _billItems) {
      ...
      for (var contactId in item.participantContactIds) {
        final participant = _selectedParticipants.firstWhere(
          (p) => p.id == contactId,
        );
        cost[participant.mainName] =
            (cost[participant.mainName] ?? 0.0) + costPerPerson;
      }
    }
      ...
      body: FutureBuilder(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            return const Center(
              child: Text('เกิดข้อผิดพลาดในการโหลดข้อมูลเริ่มต้น'),
            );
          }
          return Column(
            children: [
        ...
        children: [
          ...
          if (unselectedContacts.isNotEmpty)
            TextButton.icon(
              ...
              label: Text(
                'เลือกจากรายชื่อหลักที่เหลือ (${unselectedContacts.length} คน)',
              ),
            ),
        ],
      ),
  ...
  void _addItem() async {
    ...
    final tempParticipants = _selectedParticipants
        .map(
          (c) => Participant(
            contactId: c.id,
            name: c.mainName,
